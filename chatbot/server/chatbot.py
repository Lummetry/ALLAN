...
class Server:
    ...
    def __init__(self, bot_name, config_file, show_bot_messages=True):
        ...
        self.logger = Logger(lib_name="ALLANBOT-SERVER", config_file=config_file,
                             TF_KERAS=False,
                             HTML=False)
        self.profile = []
        ...
        with open(self.logger.GetDataFile(self.CONFIG['VULGARITIES']), 'rt', encoding='utf-8') as handle:
    ...
    def get_hashtags(self):
        with open(self.logger.GetDataFile(self.CONFIG['HASHTAGS']), 'r') as f:
    ...
    def _send_message(self):
        ...
        if self.CONV_STATE == 1:
            ...
            if not os.path.exists(folder_conversations):
                os.mkdir(folder_conversations)
            with open(folder_conversations + '/{}.txt'.format(self.CONV_PREFIX), 'wt') as f:
