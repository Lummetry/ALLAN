...
if __name__ == '__main__':
  ...
  parser.add_argument("-c", "--config", help="Configuration file in JSON format",
                      type=str, default='chatbot/02_config_dif_emb_no_bot_intent_imobiliare.txt')
  args = parser.parse_args()

  log_data = Logger(lib_name='ALLANBOT',
                    config_file='chatbot/config_check_ready.txt',
                    TF_KERAS=False)
  texts_folder = log_data.GetDataSubFolder(log_data.config_data['TEXTS_FOLDER'])
  labels_folder = log_data.GetDataSubFolder(log_data.config_data['LABELS_FOLDER'])
  ...
  d.GenerateLabels(labels_folder)

  batches_train = d.GenerateBatches(texts_folder,
                                    use_characters=True,
                                    use_labels=True, eps_words=7, eps_characters=21)
  batches_val = None
