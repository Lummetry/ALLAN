...
class ChatBot:
  ...
  def _parse_config_data(self):
    ...
    fn_inv_user_labels_dictionary = self.config_data['INV_USER_LABELS_DICTIONARY']

    with open(fn_inv_user_labels_dictionary, 'rb') as handle:
      self.inv_user_labels_dictionary = pickle.load(handle)
  ...
  def _step_by_step_prediction(self, data_loader, _input, method='sampling', verbose=1):
    ...
    label = np.expand_dims(np.argmax(predict_results[-1], axis=-1), axis=-1)
    dec_model_inputs = []
    ...
    label = self.inv_user_labels_dictionary[label[0,0]]
    if verbose:
      self._log("  --> '{}' ({})".format(predicted_text, label))
